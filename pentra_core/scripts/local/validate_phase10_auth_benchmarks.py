"""Live local validation harness for Phase 10 P3A.2 against benchmark targets."""
...
OUTPUT_PATH = OUTPUT_DIR / "auth_benchmarks_live_latest.json"

_AUTH_EXPECTATION_TYPES = {
    "auth_bypass",
    "idor",
...
def _top_route_assessments(discovery: dict[str, Any], limit: int = 10) -> list[dict[str, Any]]:
    capability = discovery.get("multi_role_stateful_auth_capability") or {}
...
def _top_candidates(discovery: dict[str, Any], limit: int = 10) -> list[dict[str, Any]]:
    candidates = discovery.get("auth_candidates") or []
    ...
    ranked = sorted(
        ...
        key=lambda item: (
            ...
            -int(str(item.get("vulnerability_type") or "").strip() in {"privilege_escalation", "auth_bypass"}),
            str(item.get("route_group") or ""),
        ),
    )
    return ranked[:limit]
...
def _filter_auth_expectations(target_spec: dict[str, Any]) -> list[str]:
    expectations = {}
    coverage = target_spec.get("pack_coverage_expectations")
    if isinstance(coverage, dict):
        expectations = coverage.get("p3a_multi_role_stateful_auth") if isinstance(coverage.get("p3a_multi_role_stateful_auth"), dict) else {}
    return [
        item
        for item in helpers.selected_paths(expectations.get("expected_vulnerability_types"))
        if item in _AUTH_EXPECTATION_TYPES
    ]


def _evaluate_auth_assessment(
    *,
    target_spec: dict[str, Any],
    discovery: dict[str, Any],
) -> dict[str, Any]:
    expected_types = set(_filter_auth_expectations(target_spec))
    candidates = discovery.get("auth_candidates") or []
    detected_types = {
    ...
    coverage = target_spec.get("pack_coverage_expectations")
    pack_expectations = (
        coverage.get("p3a_multi_role_stateful_auth")
        if isinstance(coverage, dict) and isinstance(coverage.get("p3a_multi_role_stateful_auth"), dict)
        else {}
    )
    ...
    return {
        "scope": "multi_role_stateful_auth_only",
        ...
        "role_count": int(capability.get("role_count") or 0),
    }
...
async def _run_target(target_spec: dict[str, Any]) -> dict[str, Any]:
    ...
    assessment = _evaluate_auth_assessment(target_spec=target_spec, discovery=discovery)
    record = {
        "benchmark_key": str(target_spec.get("key") or ""),
        "captured_at": helpers.utc_now(),
        ...
        "multi_role_stateful_auth_capability": discovery.get("multi_role_stateful_auth_capability") or {},
        "auth_candidates": discovery.get("auth_candidates") or [],
        "auth_assessment": assessment,
        "top_candidates": _top_candidates(discovery),
        "top_route_assessments": _top_route_assessments(discovery),
    }
    per_target_path = OUTPUT_DIR / f"{record['benchmark_key']}_auth_live_latest.json"
